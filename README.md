...
卡片界面语言跟随 dsh 页面（读 `<html lang>` 并实时响应，中/英自动切换）。
...
拖动中面板/球直接贴合指针（钳位不介入），松手与窗口 resize 时才收敛：
球体始终完整留在视口内（贴边允许、不留隐性边距），展开面板则优先翻转
展开方向（球在右/下半屏就向左/上展开）、仍出界再 clamp 进 10px 边距
（宽高上限收到视口内）；钳位只影响面板显示，不改球的记忆位置。收起态的
📁 圆钮与卡片共用同一位置（卡片拖到哪儿球就在哪儿，展开也回原位），圆球
同样按住可拖——没移动过的松手才展开。卡片/圆球/预览窗渲染在
