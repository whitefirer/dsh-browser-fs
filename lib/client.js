...
function clampPanelToViewport(pos, panel, viewport) {
  return {
...
function writeStoredCardPos(anchor, panel) {
  const kept = panel === void 0 ? readStoredCardPos()?.panel ?? null : panel;
  try {
    localStorage.setItem(CARD_POS_KEY, JSON.stringify(kept === null ? { left: anchor.left, top: anchor.top } : { left: anchor.left, top: anchor.top, panel: kept }));
  } catch {
  }
}
...
function createCard(source) {
  ...
  return function BrowserFsCard() {
    ...
    const [initialStored] = (0, import_react.useState)(readStoredCardPos);
    const [pos, setPos] = (0, import_react.useState)(initialStored?.anchor ?? null);
    const cardRef = (0, import_react.useRef)(null);
    const fabRef = (0, import_react.useRef)(null);
    const dragRef = (0, import_react.useRef)(null);
    const dragCleanupRef = (0, import_react.useRef)(null);
    const [dragging, setDragging] = (0, import_react.useState)(false);
    const [panelFit, setPanelFit] = (0, import_react.useState)(initialStored?.panel ?? null);
    (0, import_react.useEffect)(() => {
      const clampIntoView = () => {
    ...
    (0, import_react.useLayoutEffect)(() => {
      ...
      const target = panelFit === null ? fitPanelToViewport(
    ...
    const onHandlePointerDown = (event) => {
      ...
      const drag = {
        ...
        form: el === cardRef.current ? "card" : "fab",
      ...
      const settle = () => {
        ...
        if (drag.form === "card") {
          const card = cardRef.current;
          const settledPanel = clampPanelToViewport(
            rawPos,
          ...
          writeStoredCardPos(anchor, void 0);
        }
      };
      const onUp = (e) => {
        if (e.pointerId !== drag.pointerId) return;
